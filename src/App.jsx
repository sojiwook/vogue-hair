...
function Btn({ children, onClick, variant = "primary", size = "md", disabled, full, style = {} }) {
  const sz = size === "sm" ? { fontSize: 12, padding: "7px 14px" }
    : size === "lg" ? { fontSize: 15, padding: "14px 28px" }
    : { fontSize: 13, padding: "10px 20px" };
  const vr = variant === "gold" ? { background: C.gold, color: "#fff", boxShadow: "0 4px 16px rgba(184,150,90,0.3)" }
    : variant === "kakao" ? { background: C.kakao, color: "#3a1d00" }
    : variant === "ghost" ? { background: "transparent", color: C.sub, border: `1px solid ${C.border}` }
    : variant === "outline" ? { background: "#fff", color: C.gold, border: `1px solid ${C.gold}` }
    : { background: C.text, color: "#fff" };
  return (
    <button onClick={onClick} disabled={disabled} style={{
      ...sz, ...vr, border: vr.border || "none", borderRadius: 10,
    ...
    <div style={{
      ...
      <div style={{ width: 380 }}>
        ...
        <Card style={{ boxShadow: "0 8px 40px rgba(0,0,0,0.08)" }}>
          ...
          <Btn variant="gold" size="lg" full onClick={() => {
            if (id === "admin" && pw === "vogue2026") onLogin();
            else setErr("아이디 또는 비밀번호를 확인해주세요.");
          }}>로그인 →</Btn>
        </Card>
      </div>
    </div>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div>
          <h2 style={{ fontSize: 22, fontWeight: 900, marginBottom: 4 }}>고객 관리</h2>
          <p style={{ fontSize: 13, color: C.muted }}>총 {list.length}명</p>
        </div>
          ...
          {filtered.map(c => {
            ...
            return (
              <div key={c.id} onClick={() => onSelect(c)}
                style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "16px 20px", cursor: "pointer", display: "flex", alignItems: "center", gap: 16, transition: "all 0.18s" }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = C.gold; e.currentTarget.style.boxShadow = "0 4px 20px rgba(184,150,90,0.12)"; }}
                ...
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  <p style={{ fontSize: 11, color: C.muted }}>최근 방문</p>
                  <p style={{ fontSize: 12, fontWeight: 700 }}>{latest?.date ?? "-"}</p>
                </div>
...
function AddCustomer({ onSave, onCancel }) {
  ...
  const save = async () => {
    ...
    const { data, error } = await supabase.from("customers").insert({
      name: form.name, phone: form.phone, age: Number(form.age) || 0,
      stylist: form.stylist, memo: form.memo,
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
        ...
        <h2 style={{ fontSize: 20, fontWeight: 900 }}>신규 고객 등록</h2>
      </div>
      <Card style={{ maxWidth: 540 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
          ...
          <div>
            ...
            <select value={form.stylist} onChange={e => set("stylist", e.target.value)}
              style={{ width: "100%", padding: "10px 14px", border: `1px solid ${C.border}`, borderRadius: 9, fontSize: 13, fontFamily: "inherit", outline: "none" }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <textarea value={form.memo} onChange={e => set("memo", e.target.value)} placeholder="알러지, 선호사항..."
            style={{ width: "100%", height: 70, padding: "10px 14px", border: `1px solid ${C.border}`, borderRadius: 9, fontSize: 13, fontFamily: "inherit", resize: "none", outline: "none", boxSizing: "border-box" }} />
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <Btn variant="ghost" onClick={onCancel} style={{ flex: 1 }}>취소</Btn>
          <Btn variant="gold" onClick={save} disabled={saving} style={{ flex: 2 }}>{saving ? "저장 중..." : "✓ 등록 완료"}</Btn>
        </div>
      </Card>
    </div>
  );
      ...
      <Card style={{ marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            {images.length > 0 && (
              <Btn variant="gold" size="sm" onClick={() => images.forEach((_, i) => {
                if (!images[i].done && !images[i].analyzing) analyze(i);
              })}>🔬 전체 분석</Btn>
            )}
          </div>
        </div>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            {images.map((img, i) => (
              <div key={i} onClick={() => setActiveIdx(i)}
                style={{ position: "relative", width: 82, height: 82, borderRadius: 10, overflow: "hidden", border: `2px solid ${activeIdx === i ? C.gold : C.border}`, cursor: "pointer" }}>
                ...
                <button onClick={e => { e.stopPropagation(); setImages(p => p.filter((_, j) => j !== i)); setActiveIdx(0); }}
                  style={{ position: "absolute", top: 2, left: 2, width: 16, height: 16, borderRadius: "50%", background: "rgba(0,0,0,0.5)", border: "none", color: "#fff", fontSize: 10, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
              </div>
            ))}
            {images.length < 5 && (
              <div onClick={() => fileRef.current.click()}
                style={{ width: 82, height: 82, borderRadius: 10, border: `2px dashed ${C.border}`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", background: C.bg }}>
...
function HistoryTab({ customer, onAddVisit }) {
  ...
  const save = async () => {
    ...
    const { data, error } = await supabase.from("visits").insert({
      customer_id: customer.id, date: form.date, service: form.service,
      sleep: Number(form.sleep), stress: Number(form.stress),
      moisture: Number(form.moisture), elasticity: Number(form.elasticity),
      score: score(form), note: form.note,
    }).select().single();
    setSaving(false);
    if (error) { alert("저장 실패: " + error.message); return; }
    onAddVisit(data);
    ...
    <div>
      ...
      {showForm && (
        <Card style={{ marginBottom: 16, background: C.goldBg, border: `1px solid ${C.goldLight}` }}>
          ...
          <textarea value={form.note} onChange={e => set("note", e.target.value)} placeholder="스타일리스트 메모..."
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  {i === 0 && <span style={{ fontSize: 10, background: C.goldBg, color: C.gold, border: `1px solid ${C.goldLight}`, padding: "2px 8px", borderRadius: 99, fontWeight: 700 }}>최근</span>}
                </div>
  ...
  <div style={{ marginTop: 10, background: "#f8f6f2", borderRadius: 8, padding: "10px 14px" }}>
    ...
    <p style={{ fontSize: 12, color: C.sub, lineHeight: 1.8, whiteSpace: "pre-wrap" }}>{v.scalp_report}</p>
        ...
        {visits.length === 0 && (
          <div style={{ textAlign: "center", padding: 40, color: C.muted }}>아직 방문 기록이 없습니다.</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
      <Card>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          {TYPES.map(t => (
            <button key={t.id} onClick={() => setMsgType(t.id)} style={{
              flex: 1, padding: "9px 6px", border: `1px solid ${msgType === t.id ? C.gold : C.border}`,
              borderRadius: 9, background: msgType === t.id ? C.goldBg : "#fff",
        ...
        {done && !sent && <Btn variant="kakao" onClick={() => setSent(true)} full>📨 카카오톡 발송</Btn>}
        {sent && (
          <div style={{ background: "#edf7f1", border: "1px solid #a8d5b5", borderRadius: 10, padding: "12px 16px", textAlign: "center" }}>
            <p style={{ color: C.green, fontWeight: 800 }}>✅ 발송 완료!</p>
            <p style={{ color: C.sub, fontSize: 12, marginTop: 3 }}>{customer.name}님께 전송됨</p>
          </div>
        )}
      </Card>
      <Card>
              ...
              <div>
                ...
                <div style={{ background: "#fff", borderRadius: "0 14px 14px 14px", padding: "10px 12px" }}>
                  <p style={{ fontSize: 12, color: "#1a1a1a", lineHeight: 1.8, whiteSpace: "pre-wrap" }}>
                    {msg}{!done && <span style={{ color: C.gold }}>▋</span>}
                  </p>
                </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>
            담당: {customer.stylist} · 등록일: {customer.join_date} · 총 {visits.length}회 방문
...
export default function App() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const merged = (cData || []).map(c => ({
        ...c,
        visits: (vData || []).filter(v => v.customer_id === c.id),
      }));
      setCustomers(merged);
    } catch (e) {
      console.error(e);
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Noto Sans KR', 'DM Sans', sans-serif", color: C.text }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;600;700;800;900&family=Noto+Sans+KR:wght@400;700;800;900&display=swap');
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ::-webkit-scrollbar { width: 5px; }
        ::-webkit-scrollbar-thumb { background: #e0d8cc; border-radius: 99px; }
      ...
      <div style={{ background: "#fff", borderBottom: `1px solid ${C.border}`, position: "sticky", top: 0, zIndex: 100, boxShadow: "0 1px 12px rgba(0,0,0,0.05)" }}>
        <div style={{ maxWidth: 980, margin: "0 auto", padding: "0 24px", height: 58, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <p style={{ fontSize: 13, fontWeight: 900 }}>{SHOP.name}</p>
              <p style={{ fontSize: 9, color: C.muted, letterSpacing: "0.08em" }}>HAIRCARE HEALTH PLATFORM</p>
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div style={{ maxWidth: 980, margin: "0 auto", padding: "28px 24px 60px" }}>
        {page === "list" && (
          <CustomerList
            customers={customers}
            onSelect={c => { setSelected(c); setPage("detail"); }}
            onAdd={() => setPage("add")}
            loading={loading}
          />
        )}
        {page === "add" && (
          <AddCustomer
            onSave={c => { setCustomers(p => [c, ...p]); setPage("list"); }}
